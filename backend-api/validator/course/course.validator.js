...
const createCourseValidator = [
    ...
    body("access_for_verified_user").isBoolean().withMessage("access_for_verified_user must be a boolean"),
    body("availability_schedule").isIn(["Immediate", "Schedule"]).withMessage('availability_schedule must be either "Immediate" or "Schedule"'),
    body("schedule_date").optional()
      .custom((value, { req }) => {
        if (req.body.availability_schedule === "Schedule" && !value) {
          throw new Error("schedule_date is required when availability_schedule is 'Schedule'");
        }
