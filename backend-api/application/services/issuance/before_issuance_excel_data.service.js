...
class BeforeIssuanceExcelDataService {
    ...
    async addBeforeIssuanceExcelDataInBulkDatabase(list = [], transaction_created_by_user_id) {
        ...
        for (const element of list) {
            ...
            if (leadProduct.data.lead_status_id === 1) {
                error_result.add({
                    error: leadProduct?.error,
                    data: element,
                    message: "Lead is not assigned to Relationship Manager.",
                });
                continue;
            }

            //TODO: Duplicate Error Check
            if (leadProduct.data.lead_status_id === 3) {
                duplicate_result.add({
                    error: leadProduct?.error,
                    data: element,
        ...
        return {
            ...
            duplicate_result: Array.from(duplicate_result),
            duplicate_count: duplicate_result.size,
        };
    }
}
