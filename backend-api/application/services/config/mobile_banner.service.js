...
class MobileBannerService {
    ...
    async insertUpdateMobileBannerService(id, title, description, is_active, file = null) {
        ...
        if (id) {
            mobileBannerDatabaseResponse =
                await this.mobileBannerDatabase.updateMobileBannerDatabase(
                    mobileBannerDatabaseResponse?.id,
                    title,
                    description,
                    is_active,
                    null,
            ...
            mobileBannerDatabaseResponse =
                await this.mobileBannerDatabase.insertMobileBannerDatabase(
                    title,
                    description,
                    is_active,
                );
        ...
        if (mobileBannerDatabaseResponse?.id && file) {
            ...
            if (uploadFileResponse) {
                const banner_url = await this.configStorage.getPublicUrl(
